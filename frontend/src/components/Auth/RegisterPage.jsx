/**
 * Register Page - OpenClaw Dark Theme
 */
...
export default function RegisterPage() {
  ...
  const { register, loading, error, clearError, oauthProviders } = useAuth();
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-bg-primary px-4 py-8 relative overflow-hidden">
      {/* Background glow */}
      ...
      <div className="w-full max-w-md relative z-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <input
                ...
                disabled={loading}
              />
              <p className="mt-2 text-xs text-text-muted">{t('auth.passwordHint')}</p>
            </div>

            <div>
              <label className="block text-sm font-semibold text-text-secondary mb-2">
                {t('auth.confirmPassword')} <span className="text-primary">*</span>
              </label>
              <input
                type="password"
        ...
        <p className="mt-8 text-center text-text-secondary text-sm">
          {t('auth.hasAccount')}{' '}
          <Link to="/auth/login" className="text-primary hover:text-primary-hover font-semibold transition-colors">
            {t('auth.loginNow')}
          </Link>
        </p>
      </div>
